...
export default function DashboardView() {
  ...
  const [isSyncingInts, setIsSyncingInts] = useState(false);
  const [pingConfirmation, setPingConfirmation] = useState(null);

  // Jira simulate states
  const [jiraEmail, setJiraEmail] = useState(user?.email || "");
  const [jiraKey, setJiraKey] = useState("TP-382");
  const [jiraSummary, setJiraSummary] = useState("Fleshed out secure auth sessions callback endpoints");
  const [isSimulatingJira, setIsSimulatingJira] = useState(false);
  ...
  const handleSendComment = async (standupId) => {
