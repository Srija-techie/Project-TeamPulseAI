    ...
    try {
      ...
      if (insights.length > 0) {
        ...
        insights.forEach((ins) => {
          ...
          doc.setFontSize(9);
          doc.setTextColor(15, 23, 42);
          const rawSummaryLines = doc.splitTextToSize(ins.summary, 170);
          ...
          if (ins.actionItems && ins.actionItems.length > 0) {
            ...
            ins.actionItems.forEach((act) => {
              ...
              splitBullet.forEach((bulletLine, idx) => {
                doc.text(bulletLine, idx === 0 ? 22 : 25, y);
                y += 4.5;
              });
            });
            y += 2;
          ...
          doc.setFontSize(9);
          doc.setTextColor(15, 23, 42);
          const wrappedBlockers = doc.splitTextToSize(blockerKeys || "No severe structural delays reported", 110);
      ...
      if (standups.length > 0) {
        if (y > 240) { doc.addPage(); y = 25; } else { y += 6; }
        ...
        standups.forEach((s) => {
          ...
          doc.setFontSize(9);
          doc.setTextColor(15, 23, 42);
          const rawYesterday = doc.splitTextToSize(s.yesterday || "None", 165);
          ...
          doc.setFontSize(9);
          doc.setTextColor(15, 23, 42);
          const rawToday = doc.splitTextToSize(s.today || "No active goals entered", 165);
          ...
          const rawBlock = doc.splitTextToSize(s.blockers || "No active standard blockers in path.", 165);
          ...
          if (s.aiSummary) {
            ...
            doc.setFontSize(8.5);
            doc.setTextColor(71, 85, 105);
            const rawAISummary = doc.splitTextToSize(s.aiSummary, 165);
